...
/*   Updated: 2018/05/25 16:18:38 by ssavchen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ldi_irr(t_cursor **cur, t_map *m_map)
{
	...
	if (ns_check_register(1, reg.r2, reg.r3))
	{
		...
		str = find_fbytes_tind(m_map, (temp->index_pos +
				((reg.index + temp->registr[reg.r2]) % IDX_MOD)));
		reg.index = unsigned_char_to_int(str);
...
void	ldi_idr(t_cursor **cur, t_map *m_map)
{
	...
	if (ns_check_register(1, 1, reg.r3))
	{
		reg.index = ns_two_bytes(m_map, temp->index_pos + 2,
								temp->index_pos + 3);
		reg.dir2 = ns_two_bytes(m_map, temp->index_pos + 4,
								temp->index_pos + 5);
		str = find_fbytes_tind(m_map, (temp->index_pos + reg.index % IDX_MOD));
		reg.index = unsigned_char_to_int(str);
		str = find_fbytes_tind(m_map, (temp->index_pos +
				((reg.index + reg.dir2) % IDX_MOD)));
		reg.index = unsigned_char_to_int(str);
		temp->registr[reg.r3] = reg.index;
	}
	temp->index_pos += 6;
...
void	ldi_drr(t_cursor **cur, t_map *m_map)
{
	...
	if (ns_check_register(1, reg.r2, reg.r3))
	{
		...
		str = find_fbytes_tind(m_map, (temp->index_pos +
				((reg.dir + temp->registr[reg.r2]) % IDX_MOD)));
		reg.index = unsigned_char_to_int(str);
		temp->registr[reg.r3] = reg.index;
	}
	temp->index_pos += 6;
